...
public:
	...
	NETVARADDOFFS(int, GetIndex, "DT_BaseEntity", "m_bIsAutoaimTarget", 0x4);
	...
	Vector getViewmodelAttachment(int number);
	int AttachmentSlot(C_BaseViewModel* viewmodel);
	bool IsPlayer();
	bool IsLoot();
	bool IsGrenade();
	bool IsWeapon();
...
class C_BaseToneMapController : public C_BaseEntity
{
public:
...
public:
	...
	NETVAR(int, GetPrecipitationType, "DT_Precipitation", "m_nPrecipType");
};
...
public:
	...
	bool HasBullets();
	bool CanFire();
	int getWeaponIndex(int weaponId);
	int getWeaponStandalone(int weaponId);
	int getWeaponClass(int weaponId);
	float getWeaponPenetration(int weaponId);
	bool IsGrenade();
	bool IsKnife();
	bool IsReloading();
	int GetType();
...
public:
	...
	Vector GetAbsOrigin() {
		...
		return CallVFunction<original_fn>(this, 9)(this);
	}
	...
	bool visibleTo(C_BasePlayer* player, C_BasePlayer* other);
	...
	int m_nMoveType();
	QAngle * GetVAngles();
	void SetVAngles(QAngle vangles);
