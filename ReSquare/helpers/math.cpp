...
namespace Math
{
    ...
    //--------------------------------------------------------------------------------    
    Vector Indicators(Vector origin, int length, float yaw)
    {
        float rotation = DEG2RAD(yaw-45);

        float cos_rot = cos(rotation);
        float sin_rot = sin(rotation);

        float new_x = cos_rot * length - sin_rot * length;
        float new_y = sin_rot * length + cos_rot * length;
        return Vector(origin.x+new_x, origin.y+new_y, origin.z);
    }    
    //--------------------------------------------------------------------------------
    void WalktoPos(CUserCmd* cmd, float yaw, int speed)
    {
        ...
        float new_forwardmove = cos_rot * speed;
        float new_sidemove = sin_rot * speed;
    ...
    void NormalizeAngles(QAngle& angles)
    {
        ...
        while (angles.yaw > 180.0f) angles.yaw -= 360.f;
        while (angles.yaw < -180.0f) angles.yaw += 360.f;
