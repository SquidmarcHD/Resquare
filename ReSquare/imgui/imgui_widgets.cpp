...
void ImGui::EndGroupPanel()
{
    ...
    for (int i = 0; i < 4; ++i)
    {
        ...
        ImGui::GetWindowDrawList()->AddRect(
            ...
            ImColor(ImGui::GetColorU32(ImGuiCol_CheckMark)),
            halfFrame.x);
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    const ImRect check_bb({ pos.x + 3, pos.y + 3 }, { pos.x + square_sz - 3, pos.y + square_sz - 3 });
    RenderNavHighlight(total_bb, id);
    if (*v)
        RenderFrame({ check_bb.Min.x - 3, check_bb.Min.y - 3 }, { check_bb.Max.x + 3, check_bb.Max.y + 3 }, GetColorU32(ImGuiCol_PlotHistogram), true, style.FrameRounding);
    RenderFrame(check_bb.Min, check_bb.Max, GetColorU32(*v ? ImGuiCol_CheckMark : ImGuiCol_FrameBg), true, style.FrameRounding);
    ImU32 check_col = GetColorU32(ImGuiCol_CheckMark);

    ImVec2 label_pos = ImVec2(check_bb.Max.x + style.ItemInnerSpacing.x, check_bb.Min.y + style.FramePadding.y);
    if (label_size.x > 0.0f)
        RenderText({ label_pos.x, label_pos.y - 2 }, label);
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    {
        window->DrawList->AddRectFilled(ImVec2(frame_bb.Max.x - arrow_size, frame_bb.Min.y), frame_bb.Max, GetColorU32((popup_open || hovered) ? ImGuiCol_ButtonActive : ImGuiCol_ButtonHovered), style.FrameRounding, (w <= arrow_size) ? ImDrawCornerFlags_All : ImDrawCornerFlags_Right);
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* v, const void* v_min, const void* v_max, const char* format, float power)
{
    ...
    const ImU32 frame_col = GetColorU32(g.ActiveId == id ? ImGuiCol_FrameBgActive : g.HoveredId == id ? ImGuiCol_FrameBgHovered : ImGuiCol_FrameBg);
    RenderNavHighlight(frame_bb, id);
    RenderFrame({ frame_bb.Min.x + 3, frame_bb.Min.y + 12 }, { frame_bb.Max.x - 3, frame_bb.Max.y - 12 }, frame_col, true, g.Style.FrameRounding);
    ...
    if (grab_bb.Max.x > grab_bb.Min.x) {
        window->DrawList->AddCircleFilled({ grab_bb.Min.x + (grab_bb.Max.x - grab_bb.Min.x) / 2, grab_bb.Min.y + (grab_bb.Max.y - grab_bb.Min.y) / 2 }, 12, GetColorU32(ImGuiCol_SliderGrab), 25);
        window->DrawList->AddCircleFilled({ grab_bb.Min.x + (grab_bb.Max.x - grab_bb.Min.x) / 2, grab_bb.Min.y + (grab_bb.Max.y - grab_bb.Min.y) / 2 }, 6, GetColorU32(ImGuiCol_SliderGrabActive), 25);
    }
    // Display value using user-provided display format so user can add prefix/suffix/decorations to the value.
    char value_buf[64];
    const char* value_buf_end = value_buf + DataTypeFormatString(value_buf, IM_ARRAYSIZE(value_buf), data_type, v, format);
    RenderTextClipped(ImVec2(frame_bb.Max.x + style.ItemInnerSpacing.x + ImGui::CalcTextSize(label).x + ImGui::CalcTextSize(" ").x, frame_bb.Min.y + style.FramePadding.y), ImVec2(frame_bb.Max.x + 100 + ImGui::CalcTextSize(label).x, frame_bb.Min.y + style.FramePadding.y + 100), value_buf, value_buf_end, NULL, ImVec2(0.f, 0.f));
...
void ImGui::PlotEx(ImGuiPlotType plot_type, const char* label, float (*values_getter)(void* data, int idx), void* data, int values_count, int values_offset, const char* overlay_text, float scale_min, float scale_max, ImVec2 frame_size)
{
    ...
    scale_min = 0;
    scale_max = ImMax(scale_max, 250.f);
    RenderFrame(frame_bb.Min, frame_bb.Max, GetColorU32(ImGuiCol_FrameBg), true, style.FrameRounding);
